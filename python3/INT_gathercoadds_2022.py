...
output_dir_coadds ='/media/rfinn/ssd4t/Halpha/reduced/virgo-coadds-int-2022/'
...
for subdir in flist1:
    ...
    if os.path.isdir(subdir) & subdir.starts('target-r_'):

        
        coadd_r = os.path.join(subdir,'coadd_r/fcoadd.fits')
        weight_r = os.path.join(subdir,'coadd_r/coadd.weight.fits')        
                                
        coadd_ha = os.path.join(subdir.replace('-r','-Halpha'),'coadd_Halpha/fcoadd.fits')
        ...
        if os.path.exists(coadd_r) & os.path.exists(coadd_ha):
