...
class getzp():
    ...
    def runse(self):
        ...
        if self.fwhm is None:
            ...
            t = f'sex {self.image} -c {defaultcat} -CATALOG_NAME {self.catdir}/{froot}.cat -BACK_SIZE 256 -BACKPHOTO_TYPE LOCAL -MAG_ZEROPOINT 0 -SATUR_LEVEL {str(ADUlimit)}'
        ...
        t = f'sex {self.image} -c {defaultcat} -CATALOG_NAME {catdir}/{froot}.cat -BACK_SIZE 256 -BACKPHOTO_TYPE LOCAL -MAG_ZEROPOINT 0 -SATUR_LEVEL {str(ADUlimit)} -SEEING_FWHM {str(fwhm)}'
        if float(fwhm) == 0:
