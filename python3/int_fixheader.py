...
parser.add_argument('--filestring', dest='filestring', default='fbtr', help='match string for input files (default =  fbtr, which operates on all fbtr*.fits files)')
parser.add_argument('--pixscalex', dest='pixelscalex', default='0.000091666666', help='pixel scale in x (default = 0.000091666666 )')
parser.add_argument('--pixscaley', dest='pixelscaley', default='0.000091666666', help='pixel scale in y (default = 0.000091666666)')
...
for f in files:
    ...
    try:
        ...
        try:
            ...
            print "WARNING: no filter info for ",f
    ...
    if 'CRPIX1' in header:
        header['CRPIX1']=(1024.,'X reference pixel')
        header['CRPIX2']=(2048.,'Y reference pixel')
    else:
        header.append(card=('CRPIX1',1024.,'X reference pixel'))
    ...
    if 'CD1_2' in header:
        ...
        header.append(card=('CD1_2',0,''))# value from Becky's quick_WCS_instructions
        header.append(card=('CD2_1',0,''))# value from Becky's quick_WCS_instructions
    if 'CTYPE1' in header:
    ...
    if 'GAIN' in header:
        header['GAIN']=('2.9','gain (e-/ADU)')
    else:
        header.append(card=('GAIN','2.9','gain (e-/ADU)'))
